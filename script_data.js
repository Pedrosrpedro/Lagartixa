...
// NOVO PARA DEBUG: Nosso console visual
function debugLog(message) {
    ...
    if (list) {
        ...
        list.scrollTop = list.scrollHeight; // Rola para a mensagem mais recente
    }
}
debugLog("Debug Ativado. script_data.js carregado.");

// --- OBJETOS DE DADOS E CONFIGURAÇÕES GLOBAIS ---
let gecko = {};
const temperamentos = {
...
const lifeStages = { filhote: { name: "Filhote", nextStageDay: 5, spriteKey: "filhote", decayMod: 1.2 }, jovem: { name: "Jovem", nextStageDay: 15, spriteKey: "jovem", decayMod: 1.0 }, adulto: { name: "Adulto", nextStageDay: 30, spriteKey: "adulto", decayMod: 0.9 }, anciao: { name: "Ancião", nextStageDay: Infinity, spriteKey: "anciao", decayMod: 1.1 } };
const seasons = { primavera: { name: "Primavera", icon: "icone_estacao_primavera.png", class: "season-spring", effects: () => gecko.happiness = Math.min(100, gecko.happiness + 0.1) }, verao: { name: "Verão", icon: "icone_estacao_verao.png", class: "season-summer", effects: () => gecko.decayRates.thirst *= 1.3 }, outono: { name: "Outono", icon: "icone_estacao_outono.png", class: "season-autumn", effects: () => {} }, inverno: { name: "Inverno", icon: "icone_estacao_inverno.png", class: "season-winter", effects: () => { gecko.decayRates.hunger *= 1.3; gecko.decayRates.happiness *= 1.1; } } };
const moods = { contente: { name: "Contente", icon: "humor_contente.png" }, entediado: { name: "Entediado", icon: "humor_entediado.png" }, estressado: { name: "Estressado", icon: "humor_estressado.png" } };
const habitats = { pote: { name: "Pote de Vidro", furnishingLimit: 4, bg_day: 'fundo_pote_dia.png', bg_night: 'fundo_pote_noite.png', nextUpgrade: 'bacia' }, bacia: { name: "Bacia Espaçosa", furnishingLimit: 7, bg_day: 'fundo_bacia_dia.png', bg_night: 'fundo_bacia_noite.png', cost: 500, passiveBonus: () => gecko.decayRates.happiness *= 0.9, nextUpgrade: 'terrario' }, terrario: { name: "Terrário Panorâmico", furnishingLimit: 12, bg_day: 'fundo_terrario_dia.png', bg_night: 'fundo_terrario_noite.png', cost: 1200, passiveBonus: () => { gecko.decayRates.happiness *= 0.8; gecko.decayRates.hunger *= 0.95; }, nextUpgrade: null } };
const ingredients = { minhoca: { name: "Minhoca Seca", sprite: "ingrediente_minhoca.png" }, broto: { name: "Broto Verde", sprite: "ingrediente_broto.png" }, fruta: { name: "Fruta Silvestre", sprite: "ingrediente_fruta.png" } };
const recipes = { salada: { name: "Salada de Brotos", sprite: "refeicao_salada.png", ingredients: { broto: 2 }, hunger: 25, happiness: 10 }, espeto: { name: "Espeto de Minhoca", sprite: "refeicao_espeto.png", ingredients: { minhoca: 1, fruta: 1 }, hunger: 40, happiness: 20 } };
const shopItems = { remedio: { name: "Remédio", cost: 200, sprite: "icone_doente.png", desc: "Cura qualquer doença." } };
const furnishings = { poster1: { name: "Pôster Lagartinho", cost: 50, sprite: "item_poster_1.png", type: 'wall', size: { w: 50, h: 70 } }, poster2: { name: "Pôster Pôr do Sol", cost: 50, sprite: "item_poster_2.png", type: 'wall', size: { w: 70, h: 50 } }, caverna: { name: "Caverna Aconchegante", cost: 300, sprite: "item_caverna.png", type: 'floor', size: { w: 120, h: 80 }, interactive: true, action: 'use-cave' }, piscina: { name: "Piscina de Pedra", cost: 300, sprite: "item_piscina.png", type: 'floor', size: { w: 100, h: 60 }, interactive: true, action: 'use-pool' }, lampada: { name: "Lâmpada de Aquecimento", cost: 400, sprite: "item_lampada.png", type: 'wall', size: { w: 80, h: 80 }, interactive: false, requiredHabitat: 'terrario' }, tronco: { name: "Tronco Grande", cost: 350, sprite: "item_tronco.png", type: 'floor', size: { w: 150, h: 90 }, interactive: false, requiredHabitat: 'terrario' } };
const jarUpgrades = { filtro: { name: "Filtro de Água", cost: 150, desc: "Sede diminui 20% mais devagar." }, substrato_areia: { name: "Areia do Deserto", cost: 120, img: "substrato_areia.png" }, substrato_terra: { name: "Terra Fofa", cost: 120, img: "substrato_terra.png" }, substrato_musgo: { name: "Musgo Úmido", cost: 120, img: "substrato_musgo.png" } };
const cosmetics = { default: { name: "Padrão", cost: 0, class: 'bg-default'}, floresta: { name: "Floresta", cost: 100, class: 'bg-floresta' }, praia: { name: "Praia", cost: 100, class: 'bg-praia' }, cidade: { name: "Cidade", cost: 100, class: 'bg-cidade' } };
const legacyUpgrades = { sorte: { name: "Sorte Herdada", cost: 5, maxLevel: 3, desc: "Aumenta a chance de eventos bons." }, instinto: { name: "Instinto de Sobrevivência", cost: 10, maxLevel: 3, desc: "Comece com status de fome e sede mais altos." }, carisma: { name: "Carisma Genético", cost: 8, maxLevel: 3, desc: "Ganhe pontos 10% mais rápido." } };
const achievements = { sobrevivente: { name: "Sobrevivente Novato", desc: "Sobreviva por 10 dias.", goal: 10, reward: 50, tracker: () => gecko.days }, gourmet: { name: "Gourmet", desc: "Prepare 10 refeições.", goal: 10, reward: 30, tracker: () => gecko.stats.timesFed }, comprador: { name: "Decorador de Potes", desc: "Coloque 5 móveis no pote.", goal: 5, reward: 100, tracker: () => gecko.placedFurnishings.length }, carinhoso: { name: "Cheio de Carinho", desc: "Faça carinho 50 vezes.", goal: 50, reward: 40, tracker: () => gecko.stats.timesPetted }, reiDaMosca: { name: "Rei da Mosca", desc: "Pegue 10 moscas em uma única partida.", goal: 10, reward: 75, tracker: () => gecko.stats.bestMinigameScore } };
const eventosAleatorios = [ { nome: "Mosca no Pote", chance: 0.1, efeito: () => { gecko.happiness = Math.min(100, gecko.happiness + 20); gecko.hunger = Math.min(100, gecko.hunger + 10); logMessage('evento_bom'); messageLog.textContent += " (Uma mosca entrou e virou lanche!)"; playSound('happy'); } }, { nome: "Sombra Assustadora", chance: 0.1, efeito: () => { gecko.happiness = Math.max(0, gecko.happiness - 15); logMessage('evento_ruim'); messageLog.textContent += " (Uma sombra passou e assustou ela!)"; playSound('sad'); } }, { nome: "Sol Aconchegante", chance: 0.15, efeito: () => { gecko.happiness = Math.min(100, gecko.happiness + 10); logMessage('evento_bom'); messageLog.textContent += " (Um raio de sol aqueceu o pote.)"; playSound('happy'); } }, ];
debugLog("Objetos de dados criados.");
...
debugLog("Elementos do DOM cacheados.");
...
// --- FUNÇÕES DE FLUXO DE JOGO E ESTADO (SAVE, LOAD, MENUS) ---
function createNewGecko(name, legacyData) {
    debugLog("Criando nova lagartixa...");
    const keys = Object.keys(temperamentos);
    const randomKey = keys[Math.floor(Math.random() * keys.length)];
    let newGecko = { name: name, health: 100, hunger: 100, thirst: 100, happiness: 100, pontos: 20, days: 0, temperamento: temperamentos[randomKey], lifeStage: 'filhote', currentSeason: 'primavera', currentMood: 'contente', currentHabitat: 'pote', currentSubstrate: 'none', currentBackground: 'bg-default', isSick: false, isInjured: false, recoveryProgress: 0, canBeReleased: true, isInteracting: false, lastPetTime: 0, stats: { timesFed: 0, timesPetted: 0, bestMinigameScore: 0, lastActionDay: 0 }, skills: { forager: { level: 1, xp: 0 }, athlete: { level: 1, xp: 0 } }, inventory: { ingredients: {}, furnishings: [] }, placedFurnishings: [], ownedUpgrades: [], achievements: JSON.parse(JSON.stringify(achievements)), diaryLog: [`Dia 0: Nasci neste ${habitats.pote.name}.`], currentWhim: null, legacyData: legacyData || { points: 0, upgrades: {} }, decayRates: { hunger: 0.5, thirst: 0.8, happiness: 0.5, health: 1.0 } };
    ...
    debugLog(`Lagartixa "${name}" criada com sucesso.`);
    return newGecko;
}
...
function saveGame(showMsg = false) {
    ...
    if (showMsg) {
        ...
        debugLog("Jogo salvo manualmente.");
    }
}
...
function loadGame() {
    ...
    if (savedData) {
        ...
        debugLog("Save carregado.");
    } else {
        debugLog("Nenhum save encontrado.");
    }
}

function setupMainMenu() {
    debugLog("Configurando menu principal...");
...
function startGame(isNew, name) {
    debugLog(`Iniciando jogo. É novo? ${isNew}. Nome: ${name}`);
